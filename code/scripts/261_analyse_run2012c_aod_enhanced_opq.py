...
IN = ROOT / "outputs_run2012c_aod_enhanced_validation"
OUT = ROOT / "outputs_run2012c_aod_enhanced_opq_analysis"
TABLES = OUT / "tables"
REPORTS = OUT / "reports"
LEDGER = IN / "tables" / "01_run2012c_aod_enhanced_extraction_ledger.csv"
...
def main() -> None:
    ...
    for row in ledger.itertuples(index=False):
        if row.status not in {"completed", "existing"}:
            continue
        frame = pd.read_csv(row.output_path, low_memory=False)
        frames.append(frame)
    if not frames:
        raise RuntimeError("No completed enhanced Run2012C AOD feature files found.")
    raw = pd.concat(frames, ignore_index=True)
    raw.to_csv(TABLES / "01_run2012c_aod_enhanced_raw_combined.csv", index=False)
    ...
    events.to_csv(TABLES / "02_run2012c_aod_enhanced_scored_events.csv", index=False)

    feature_audit = pd.DataFrame(
        [
            {
                "rows": len(events),
                "btag_status_nonzero_fraction": float((pd.to_numeric(events["btag_discriminator_status"], errors="coerce").fillna(0) > 0).mean()),
                "medium_btag_nonzero_fraction": float((pd.to_numeric(events["N_btags_medium"], errors="coerce").fillna(0) > 0).mean()),
                "secondary_vertex_status_nonzero_fraction": float((pd.to_numeric(events["secondary_vertex_status"], errors="coerce").fillna(0) > 0).mean()),
                "secondary_vertex_nonzero_fraction": float((pd.to_numeric(events["secondary_vertex_count"], errors="coerce").fillna(0) > 0).mean()),
                "packed_candidate_nonzero_fraction": float((pd.to_numeric(events["packed_candidate_count"], errors="coerce").fillna(0) > 0).mean()),
            }
        ]
    )
    feature_audit.to_csv(TABLES / "03_run2012c_aod_enhanced_feature_audit.csv", index=False)

    tagged = tag_microbands(events)
    counts = tagged.groupby(["primary_dataset", "jet_bin", "microband"], observed=False).size().reset_index(name="n")
    counts.to_csv(TABLES / "04_run2012c_aod_enhanced_microband_counts.csv", index=False)
    ...
    result = pd.DataFrame(
        [
            {
                "sample_validation_id": "Run2012C_AOD_enhanced_cross_era",
                "feature_scope": "enhanced_AOD_OPQ",
                ...
                "feature_mapping": "AOD btag fallbacks plus V0 secondary-vertex-like counts",
            }
        ]
    )
    result.to_csv(TABLES / "05_run2012c_aod_enhanced_opq_statistics.csv", index=False)
    ...
    if PREV_THREE.exists():
        ...
        combined_rows.append(
            {
                "combination": "three_miniaod_like_samples_plus_run2012c_enhanced_aod",
                "sample_count": len(pvals),
                "fisher_statistic": float(stat),
                "fisher_p": float(comb_p),
                "fisher_Z": p_to_z(float(comb_p)),
                "enhanced_aod_included_as_stress_only": True,
            }
        )
    combined = pd.DataFrame(combined_rows)
    combined.to_csv(TABLES / "06_run2012c_added_enhanced_cross_era_combination_stress.csv", index=False)

    report = f"""# Run2012C Enhanced AOD OPQ Cross-Era Validation
...
{feature_audit.to_markdown(index=False, floatfmt='.6g')}
...
"""
    (REPORTS / "01_RUN2012C_AOD_ENHANCED_OPQ_CROSS_ERA_VALIDATION.md").write_text(report, encoding="utf-8")
    print(result.to_string(index=False))
    print(feature_audit.to_string(index=False))
    if not combined.empty:
        print(combined.to_string(index=False))
    print(REPORTS / "01_RUN2012C_AOD_ENHANCED_OPQ_CROSS_ERA_VALIDATION.md")
